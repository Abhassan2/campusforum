...
function PostCard({ post }) {
  ...
  return (
    <div
      className="flex flex-col border-t-4 border-neutral-200 md:flex-row "
    >
      {/* post header */}
      ...
      {post?.media?.some((m)=> m.type.includes("image")) && (
        <div className="w-full aspect-video md:w-110">
          <Image
      ...
      {post?.media?.some((m)=> m.type.includes("video")) && (
        <div className="relative md:w-110 md:max-w-150 ">
          <video
            ref={videoRef}
            autoPlay
            loop
            muted
            className="w-full max-h-115 md:max-h-175 object-fill"
          >
            <source
              src={post?.media[0]?.url || "/landing_img.png"}
              type="video/mp4"
